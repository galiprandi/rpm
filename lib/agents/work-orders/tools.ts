...
export const searchWorkOrdersTool = tool({
  description: 'Busca órdenes de trabajo por estado, cliente o vehículo.',
  inputSchema: z.object({
    status: z.string().optional().describe('Filtrar por estado (WAITING, CONFIRMED, IN_PROGRESS, QC_CHECK, READY, PAID, DELIVERED)'),
  ...
  execute: async ({ status, customerId, limit }) => {
    logger.debug({ status, customerId }, 'Search work orders');
    ...
    const orders = await prisma.work_order.findMany({
      ...
      include: {
        ...
        vehicle: { select: { id: true, identifier: true } },
    ...
    return orders.map((o) => {
      ...
      return `🔹 OT #${o.id.slice(0, 8)} - ${o.status} - ${o.customer?.name || '?'} - $${Number(o.total)}${itemCount > 0 ? ` (${itemCount} items)` : ''}`;
    }).join('\n') + `\n\n${orders.length} OT(s) encontrada(s).`;
  },
});

export const createWorkOrderTool = tool({
  description: 'Crea una orden de trabajo (OT). Requiere ID del cliente y del vehículo. Opcionalmente puede incluir items y fecha programada.',
  inputSchema: z.object({
    ...
    scheduledDate: z.string().optional().describe('Fecha programada (ISO string)'),
  }),
  execute: async (input) => {
    logger.debug({ customerId: input.customerId, vehicleId: input.vehicleId }, 'Create work order');
...
export const updateWorkOrderStatusTool = tool({
  description: 'Actualiza el estado de una orden de trabajo. Estados válidos: WAITING, CONFIRMED, IN_PROGRESS, QC_CHECK, READY, PAID, DELIVERED.',
  inputSchema: z.object({
    workOrderId: z.string().describe('ID de la orden de trabajo'),
    status: z.enum(['WAITING', 'CONFIRMED', 'IN_PROGRESS', 'QC_CHECK', 'READY', 'PAID', 'DELIVERED']).describe('Nuevo estado'),
  }),
  execute: async (input) => {
    logger.debug({ workOrderId: input.workOrderId, status: input.status }, 'Update work order status');

    try {
      const updated = await updateWorkOrder(input.workOrderId, { status: input.status }, {
        userId: 'bot',
        userEmail: 'nitro@rpm',
      });
...
export const getWorkOrderDetailTool = tool({
  ...
  execute: async ({ workOrderId }) => {
    ...
    const itemsStr = wo.work_order_item.map((i) =>
