...
export const getCashStatusTool = tool({
  description: 'Consulta el estado actual de la caja: resumen del día con ingresos, egresos y saldo.',
...
export const getTodaySummaryTool = tool({
  description: 'Resumen del día actual: ventas, gastos, movimientos de caja y órdenes de trabajo.',
  inputSchema: z.object({}),
  execute: async () => {
...
export const createDirectSaleTool = tool({
  description: 'Registra una venta directa (mostrador). Requiere producto, cantidad, precio unitario, nombre del cliente y método de pago.',
  inputSchema: z.object({
    ...
    paymentMethodId: z.string().describe('ID del método de pago'),
    customerName: z.string().describe('Nombre del cliente (o "Mostrador" para venta sin cliente)'),
    customerId: z.string().optional().describe('ID del cliente (opcional)'),
    notes: z.string().optional().describe('Notas de la venta'),
  }),
  execute: async (input) => {
    logger.debug({ productId: input.productId, quantity: input.quantity }, 'Create direct sale');
    ...
    try {
      ...
      if (Number(product.stock) < input.quantity) return `Stock insuficiente. Disponible: ${product.stock}, solicitado: ${input.quantity}`;
