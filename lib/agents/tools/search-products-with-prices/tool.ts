...
export const searchProductsWithPricesTool = tool({
  description: 'Busca productos por nombre, SKU o código de barras. Devuelve stock disponible, precio de contado y precio con tarjeta para cada producto encontrado. Usá esta tool cuando el usuario pregunte por productos, stock o precios.',
  inputSchema: z.object({
    search: z.string().describe('Término de búsqueda (nombre del producto, SKU o código de barras)'),
  ...
  execute: async ({ search, limit }) => {
    ...
    const formatted = products
      .map((p) => {
        ...
        const contado = p.contadoPrice != null ? `$${p.contadoPrice.toLocaleString('es-AR')}` : 'N/A';
        const tarjeta = p.tarjetaPrice != null ? `$${p.tarjetaPrice.toLocaleString('es-AR')}` : 'N/A';
