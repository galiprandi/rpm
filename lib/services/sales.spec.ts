...
describe('Sales Registration and Reporting', () => {
  beforeEach(() => {
    ...
    vi.mocked(prisma.work_order.aggregate).mockResolvedValue({ _sum: { total: 0 }, _count: { id: 0 } } as any);
    ...
    vi.mocked(prisma.direct_sale.aggregate).mockResolvedValue({ _sum: { total: 0 }, _count: { id: 0 } } as any);
