## 2026-07-24 - Public Website Accessibility and ARIA Standard
**Learning:** All public-facing components (including layouts, headers, footers, section grids, quick-view modals, search dialogues, and widgets) must strictly conform to accessibility compliance. Purely decorative icons and Lucide/SVG elements must be marked with `aria-hidden="true"` and standard `pointer-events-none` CSS classes to prevent screen reader noise. Interactive elements without clear textual identifiers (such as social links, search clear widgets, mobile menu triggers, and toggle buttons) must have explicit, descriptive `aria-label` attributes. Additionally, clickable parent containers (e.g. interactive grids or cards) must employ keyboard navigation support using `tabIndex={0}`, standard focus rings, semantic interactive roles (`role="button"`), and key listeners for `Enter` and `Space` keyboard clicks.
**Action:** Apply this comprehensive public accessibility protocol (ARIA labeling, decorative icon hiding, and keyboard interaction hooks) to all public-facing components and views.
## 2026-07-25 - Standardizing Dashboard Summary Card Navigation and Actionability
**Learning:** Dashboard summary card metrics should serve as intuitive navigational entry points rather than static dead-ends. When high-level summary rows are structured as a cohesive grid of cards, ensuring every single card has a clear, accessible footer link (e.g., "Ver movimientos →", "Ver reporte →") creates a consistent mental model and navigational pattern. For these footer links, providing high-contrast focus rings and explicit screen reader labels (e.g., `aria-label="Ir a la gestión de caja"`) ensures keyboard-only and assistive technology users can easily navigate between aggregate statistics and their respective detailed views.
**Action:** When designing or refactoring dashboard summary metric cards, always provide a clear, action-oriented navigational footer link with explicit `aria-label` tags and focus ring styles to maintain consistency and eliminate dead-ends.
...
## 2026-07-20 - Multi-Step Form Layout Separation & Select Trigger Accessibility
...
## 2026-07-24 - Search Input Close Buttons Keyboard Focus and Alert Contrast
**Learning:** Raw icon-only `<button>` elements absolutely positioned inside text inputs (like 'Clear Search' close buttons) are often skipped or invisible during keyboard tab-navigation because they lack visible focus-ring styles. Applying `focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none focus-visible:ring-offset-1 rounded-full p-0.5` ensures keyboard accessibility without cluttering the mouse-user visual layout. Additionally, technical validation alerts on light backgrounds must avoid `text-red-600` in favor of `text-red-700` to satisfy WCAG AA contrast (4.5:1).
**Action:** Always verify keyboard focus states for absolute close buttons inside search inputs. Ensure validation feedback alerts use 700-weight colors for contrast compliance.

## 2026-07-24 - Reports Dashboard Accessibility & Contrast Standard
**Learning:** Semantic metric text/color indications on light backgrounds inside reporting sub-modules (such as Workshop, Finance, Services, and Profitability report pages) must use level 700 text variants (e.g., `text-emerald-700` and `text-red-700`) rather than level 600 or 500 options to comply with WCAG AA (4.5:1) contrast ratios. Additionally, all decorative or non-interactive SVG/Lucide icons within these clients must be explicitly hidden from assistive technologies using `aria-hidden="true"` and standard CSS `pointer-events-none` attributes.
**Action:** When updating or designing report dashboards, elevate status indicator text colors to level 700 and add `aria-hidden="true"` + `pointer-events-none` attributes to all decorative icons.
