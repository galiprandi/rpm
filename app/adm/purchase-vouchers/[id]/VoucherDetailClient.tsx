...
export default function VoucherDetailClient({
  initialVoucher,
  ...
  const handleRemoveItem = async (itemId: string) => {
    ...
    try {
      const res = await fetch(
        `/api/purchase-vouchers/${voucher.id}/items/${itemId}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleFinalize = async () => {
    ...
    if (Math.abs(itemsTotal - declaredTotal) > 0.01) {
      ...
      const proceed = await confirm({
        ...
        description:
          "¿Está seguro de finalizar este comprobante de compra? Esta acción aumentará el stock de los productos y no podrá ser revertida.",
        confirmText: "Finalizar",
        cancelText: "Cancelar",
      });
      if (!proceed) return;
    ...
    try {
      ...
      await alert({
        ...
        description:
          "El stock de los productos ha sido incrementado correctamente.",
        variant: "success",
      });
    } catch (err: any) {
      await alert({
  ...
  const isComplete =
    Math.abs(totalCalculated - parseFloat(voucher.totalAmount)) < 0.01;
        ...
        primaryAction={
          voucher.status === "DRAFT"
            ? {
                ...
                ariaLabel:
                  "Finalizar la carga del comprobante y actualizar stock",
              }
            : undefined
        }
      >
        ...
        <div className="lg:col-span-2 space-y-6">
          {voucher.status === "DRAFT" && (
            <form
              ...
              className="bg-card border rounded-xl shadow-xs overflow-hidden"
            >
              ...
              {(voucher.items || []).length > 0 && (
                <TableBody className="bg-muted/20 border-t">
                  <TableRow className="hover:bg-transparent">
                    <TableCell
                      colSpan={3}
                      className="px-6 py-4 text-right font-medium text-muted-foreground"
                    >
                      Total Calculado:
                    </TableCell>
        ...
        <div className="space-y-6">
          <div className="bg-card border rounded-xl shadow-xs p-6 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div
                className={`p-4 rounded-lg border ${isComplete ? "bg-emerald-50 border-emerald-100" : "bg-red-50 border-red-100"}`}
              >
                <span
                  className={`text-xs uppercase tracking-wider font-semibold ${isComplete ? "text-emerald-700" : "text-red-700"}`}
                >
                  Monto Cargado
                </span>
                <div
                  className={`text-2xl font-bold mt-1 font-mono ${isComplete ? "text-emerald-700" : "text-red-700"}`}
                >
                  {totalCalculated.toLocaleString("es-AR", {
                    style: "currency",
                    currency: "ARS",
                ...
                {!isComplete && (
                  <div className="flex items-center gap-1.5 mt-2 text-xs text-red-600 font-medium font-mono">
                    ...
                    Diferencia:{" "}
                    {(
                      parseFloat(voucher.totalAmount) - totalCalculated
