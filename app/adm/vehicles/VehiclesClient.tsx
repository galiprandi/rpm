...
export default function VehiclesClient({ initialVehicles, totalVehicles }: VehiclesClientProps) {
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-semibold tracking-tight font-mono">{row.original.identifier}</div>
      ...
      },
      {
        ...
        header: "Marca/Modelo",
        cell: ({ row }) => {
          const isEquipment = ["AUDIO_EQUIPMENT", "ELECTRIC_SCOOTER", "OTHER", "TRAILER"].includes(row.original.category);
          if (isEquipment && row.original.equipmentName) {
            return <span className="font-medium">{row.original.equipmentName}</span>;
          }
          if (row.original.vehicle_make) {
            return (
              <span className="font-medium">
                {row.original.vehicle_make.name} {row.original.vehicle_model?.name || ""}
              </span>
            );
          }
      ...
      },
      {
        ...
        header: "OTs",
        cell: ({ row }) => (
          <span className="font-mono font-medium">{row.original._count.work_order}</span>
        ),
      },
    ],
    []
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center gap-2 mt-4">
          <div className="relative group">
            <Filter className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground pointer-events-none z-10" aria-hidden="true" />
      ...
      <CrudAdmin<Vehicle & { id: string }>
        ...
        loading={loading}
        columns={columns}
        emptyIcon={<Car className="h-12 w-12 mx-auto text-muted-foreground/20 mb-4" />}
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Link href={`/adm/vehicles/${vehicle.id}`}>
                  <Button variant="ghost" size="icon" className="h-8 w-8" aria-label="Ver detalle del vehículo">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Link href={`/adm/work-orders/new?vehicleId=${vehicle.id}`}>
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-primary hover:text-primary hover:bg-primary/10" aria-label="Crear nueva Orden de Trabajo">
