  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="details" className="w-full">
        ...
        <TabsContent value="details" className="pt-4 outline-none space-y-6">
          {/* Información General */}
          <Card>
            <CardHeader>
          ...
          <div className="grid md:grid-cols-3 gap-4">
                      ...
                      <div key={inv.id} className="flex items-center justify-between p-3 bg-muted rounded-lg border">
                        ...
                        <div className="flex items-center gap-2">
                          <Link href={`/adm/invoices/${inv.id}`}>
                            <button className="p-2 hover:bg-background border rounded-lg transition-colors shadow-sm" title="Ver Detalle">
                          ...
                          <button
                            ...
                            title="Descargar PDF"
                            onClick={() => window.open(`/adm/invoices/${inv.id}?print=true`, '_blank')}
                          >
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <button
                  className="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium border rounded-lg hover:bg-muted transition-colors"
                  onClick={() => generateDocument('PRESUPUESTO')}
                  disabled={!!generatingDocument}
                >
                  <Printer className="h-4 w-4 text-muted-foreground" />
                  {generatingDocument === 'PRESUPUESTO' ? 'Generando...' : 'Generar Presupuesto'}
                </button>
                <button
                  className="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium border rounded-lg hover:bg-muted transition-colors"
                ...
                >
                  <Printer className="h-4 w-4 text-muted-foreground" />
                  {generatingDocument === 'REMITO' ? 'Generando...' : 'Generar Remito'}
                </button>
                <button
                  className="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium border rounded-lg bg-secondary/50 hover:bg-secondary transition-colors"
                  onClick={() => generateDocument('INVOICE')}
                  disabled={!!generatingDocument || !!sale.invoiceId}
                >
                  <FileText className="h-4 w-4 text-muted-foreground" />
                  {generatingDocument === 'INVOICE' ? 'Generando...' : 'Generar Pre-Factura'}
                </button>
              </CardContent>
            </Card>
          </div>
