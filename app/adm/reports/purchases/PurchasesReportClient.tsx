        ...
        <div className="hidden print:block font-sans p-6 text-black bg-white" id="print-section">
          ...
          <div className="flex justify-between items-start border-b-2 border-zinc-900 pb-4 mb-6">
            ...
            <div className="text-right">
              <div className="border border-zinc-900 text-zinc-900 font-bold text-xs py-1 px-3 uppercase tracking-wider inline-block rounded">
                Reporte de Compras & Abastecimiento
              </div>
          ...
          <div className="grid grid-cols-3 gap-4 border border-zinc-300 rounded-lg p-4 mb-6 bg-zinc-50/50">
            <div className="space-y-1">
              <span className="text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Compras Totales</span>
              <p className="text-sm font-bold font-mono text-zinc-900">{formatARS(data.totalPurchases.current)}</p>
            </div>
            <div className="space-y-1">
              <span className="text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Cantidad de Comprobantes</span>
              <p className="text-sm font-bold font-mono text-zinc-900">{data.voucherCount.current}</p>
            </div>
            <div className="space-y-1">
              <span className="text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Comprobante Promedio</span>
              <p className="text-sm font-bold font-mono text-zinc-900">{formatARS(data.averageVoucher.current)}</p>
            </div>
          </div>

          {/* Dos columnas en impresión: Top Proveedores y Evolución de Compras */}
          <div className="grid grid-cols-3 gap-6 mb-6 print:break-inside-avoid">
            {/* Top Proveedores */}
            <div className="col-span-1">
              <h2 className="text-xs font-bold text-zinc-800 uppercase tracking-wider mb-2">Top Proveedores</h2>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody>
                    {data.supplierDistribution.length === 0 ? (
                      ...
                      data.supplierDistribution.slice(0, 10).map((item) => (
                        <tr key={item.supplierId} className="border-b border-zinc-200 text-[11px]">
                          <td className="p-2 font-medium truncate max-w-[120px]">{item.supplierName}</td>
                          <td className="p-2 text-right font-mono font-bold text-zinc-900">{formatARS(item.total)}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
                      ...
                      data.evolution
                        ...
                        .map((item, idx) => (
                          <tr key={idx} className="border-b border-zinc-200 text-[11px]">
                            ...
                            <td className="p-2 text-right font-mono font-bold text-red-700">{formatARS(item.total)}</td>
                            <td className="p-2 text-right">{item.count}</td>
                            <td className="p-2 text-right font-mono">{formatARS(item.count > 0 ? item.total / item.count : 0)}</td>
                          </tr>
                        ))
                    )}
                  </tbody>
          ...
          <div className="text-center border-t border-zinc-200 pt-4 text-[11px] text-zinc-400 font-mono">
            RPM Accesorios © {new Date().getFullYear()} - Reporte de Abastecimiento, Control de Proveedores y Auditoría de Costos.
          </div>
        </div>
      )}
