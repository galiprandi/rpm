...
import { getCustomers, createCustomer, type CreateCustomerInput } from "@/lib/services/customerService";
...
export const GET = withAdmin(async (request: NextRequest, _session) => {
  try {
    ...
    const result = await getCustomers({ search: search || undefined, limit, offset });
...
export const POST = withAdmin(async (request: NextRequest, _session) => {
  try {
    ...
    return NextResponse.json(
      { error: errorMessage },
      { status: errorMessage.includes("Missing") || errorMessage.includes("Invalid") ? 400 : 500 }
    );
  }
});
